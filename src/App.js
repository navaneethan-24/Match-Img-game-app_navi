...
class App extends Component {
  ...
  componentDidMount() {
    this.timerId = setInterval(this.timeStatusChange, 1000)
  }
 ...
 timeStatusChange = () => {
  ...
  if (time !== 0) {
    ...
    this.setState({ isGameEnd: true, time: 0 }) 
  }
}
  ...
  render() {
    const {isGameEnd, category, score, time, imgUrl} = this.state
        ...
        <div className="hero-container">
          {!isGameEnd && (
            <div className="gamepage-container">
              <img src={imgUrl} className="hero-image" alt="match" />
              <ul className="tab-elements">
                {tabsList.map(eachtab => (
                  <li key={eachtab.tabId}>
                    <button
                     className={`tab-button ${category === eachtab.tabId ? 'highlight-text' : ''}`}
                      type="button"
